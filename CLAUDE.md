...
- **`server.js`** wires everything: Express app + a raw `http` server + a `ws` WebSocket server used as a "Live Hub" broadcast channel (`app.get('broadcast')`, emits events like `BID_PLACED`, `CONTEST_CREATED`). The Stripe webhook route(s) are mounted with `express.raw()` *before* the global `express.json()` middleware — keep any new webhook routes above that line. CSP's `script-src` is strict (`'self'` + Cloudflare Turnstile + Stripe.js, no `unsafe-inline`/`unsafe-eval`) when `NODE_ENV=production`, and permissive otherwise — see the hardcoded header. Verified via `Content-Security-Policy-Report-Only` against the production build (home, login/Turnstile, cart) that tightening breaks nothing.
- **`src/routes/*.js`** — one file per domain (`auth`, `listings`, `users`, `payments`, `shipping`, `orders`, `notifications`, `sets`, `reviews`, `admin`, `adminBadges`, `catalog`, `analyticsTrack`, `geo`, `sitemap`, `support`, plus ClutchVault's `contest`, `wallet`, `stripe`). Mounted directly in `server.js` under `/api/<name>`.
- **Multi-TCG catalog**: beyond LEGO (`catalog.js`), there's a card-catalog system covering Magic, Yu-Gi-Oh, Lorcana, Pokémon, One Piece, Dragon Ball, and Funko. Each game gets a thin `src/routes/catalog<Game>.js` file that wraps the shared factory in `src/routes/cardCatalogRouter.js` (`/recent`, `/search`, `/:id`, with a "hybrid" local-cache + external-API deep-scan search) — mounted under `/api/catalog/<game>`.
