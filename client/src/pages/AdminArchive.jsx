    ...
    <div className="p-6 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-stone-400">Storico completo delle transazioni e degli annunci archiviati.</p>
        </div>
        <div className="bg-stone-800/50 px-4 py-2 rounded-xl border border-stone-700/50">
          <span className="text-stone-500 text-xs font-bold uppercase block">Totale Elementi</span>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-stone-900/50 border-b border-stone-800">
                ...
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500">Soggetto</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500 text-right">Prezzo</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500 text-right">Spedizione</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500 text-right text-gold-400">Fee</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500 text-right">Data</th>
                <th className="px-4 py-4 text-[10px] font-black uppercase tracking-widest text-stone-500 text-center">Azioni</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-stone-800/50">
                ...
                <tr key={item.id} className="hover:bg-stone-800/20 transition-colors group">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase ${
                      ...
                      item.status === 'expired' ? 'bg-gold-500/10 text-gold-400' : 'bg-stone-700 text-stone-300'
                    }`}>
                      {item.status}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-bold text-stone-400 uppercase w-4">S:</span>
                        <span className="text-[11px] text-white">{item.seller_username || '---'}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] font-bold text-stone-400 uppercase w-4">B:</span>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex flex-col items-end">
                      <span className="text-sm font-medium text-stone-300">€{Number(item.shipping_cost || 0).toFixed(2)}</span>
                      <span className="text-[9px] text-stone-500 uppercase">{item.selected_carrier || '---'}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {(item.status === 'expired' || item.status === 'removed') && (
                      <button 
                        ...
                        className="p-2 rounded-lg bg-gold-500/10 text-gold-400 hover:bg-gold-500 hover:text-white transition-all shadow-sm"
              ...
              {items.length === 0 && (
                <tr>
                  <td colSpan="8" className="px-4 py-12 text-center text-stone-500 text-sm">
