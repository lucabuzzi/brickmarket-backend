...
export default function Profile() {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      setPurchasesError(t('profile.purchases_load_error'));
    }
    try {
      ...
      setListingsError(t('profile.listings_load_error'));
    }
    try {
      ...
      setBidsError(t('profile.bids_load_error'));
    }
    setLoading(false);
  };
  ...
  const submitFeedback = async (e) => {
    e.preventDefault();
    if (rating === 0) return alert(t('review.rating_required_error'));
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 w-full font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div className="flex items-center gap-3">
          ...
          <SellerTypeBadge sellerType={user?.seller_type} className="!text-[0.7rem] !py-1" />
        </div>
        {canSell && (
          <Link to="/sell" className="flex items-center gap-2 bg-gold-600 hover:bg-gold-500 text-white px-5 py-2.5 rounded-lg transition-colors font-bold shadow-lg shadow-gold-600/20">
            <PlusCircle size={20} /> {t('profile.new_listing_button')}
          </Link>
        )}
      </div>
      ...
      <section className="mb-12">
        ...
        {listingsError ? (
          ...
          <div className="text-center py-12 bg-[#120f0a]/50 border border-stone-800 rounded-xl border-dashed">
            <p className="text-stone-400 mb-4">{t('profile.no_listings_yet')}</p>
            {canSell && (
              <Link to="/sell" className="inline-flex items-center gap-2 bg-stone-800 hover:bg-stone-700 text-white px-4 py-2 rounded-lg transition-colors font-bold">
                <PlusCircle size={16} /> {t('profile.create_first_listing')}
              </Link>
            )}
          </div>
        ) : (
      ...
      <section className="mb-12">
        ...
        {bidsError ? (
          ...
          <div className="text-center py-12 bg-[#120f0a]/50 border border-stone-800 rounded-xl border-dashed">
            <p className="text-stone-400">{t('profile.no_bids_yet')}</p>
          </div>
        ) : (
                  ...
                  <div className="p-4 flex-1 flex flex-col">
                    ...
                    <div className="mt-auto flex justify-between items-end">
                      ...
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {won && <span className="px-2 py-1 bg-emerald-500 text-white text-[10px] font-black rounded uppercase shadow-sm tracking-wider">{t('profile.badge_won')}</span>}
                        {item.bidStatus === 'lost' && <span className="px-2 py-1 bg-stone-800 text-stone-400 text-[10px] font-bold rounded uppercase tracking-wider">{t('profile.badge_lost')}</span>}
                        {outbid && (
      ...
      <section className="mb-12">
        <h2 className="text-xl font-black text-white mb-6 flex items-center gap-2 border-b border-stone-700/50 pb-3">
          <Package size={22} className="text-emerald-500" /> {t('profile.my_purchases_heading')}
        </h2>
        {purchasesError ? (
          <p className="text-red-500 bg-red-500/10 p-4 rounded-lg">{purchasesError}</p>
        ) : purchases.length === 0 ? (
          <div className="text-center py-12 bg-[#120f0a]/50 border border-stone-800 rounded-xl border-dashed">
            <p className="text-stone-400">{t('profile.no_purchases_yet')}</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {purchases.map(order => (
              <div key={order.id} className="bg-[#120f0a] border border-stone-700 hover:border-stone-500 rounded-xl overflow-hidden hover:scale-[1.02] transition-all duration-300 shadow-sm flex h-32">
                <div className="w-32 h-full shrink-0 bg-stone-800 border-r border-stone-800">
        ...
        <div className="fixed inset-0 bg-stone-950/80 z-[100] flex items-center justify-center p-4 backdrop-blur-sm transition-opacity">
          <div className="bg-[#120f0a] border border-stone-700 p-6 sm:p-8 rounded-2xl w-full max-w-md shadow-2xl relative overflow-hidden">
            <div className="relative z-10">
              <h3 className="text-white font-black text-2xl mb-2">{t('profile.rate_seller_title')}</h3>
              <p className="text-stone-400 text-sm mb-6 leading-relaxed">
                {t('profile.rate_seller_subtitle', { title: feedbackOrder?.listing_title })}
              </p>
              <form onSubmit={submitFeedback}>
                <div className="flex justify-center mb-8 bg-stone-900/50 py-4 rounded-xl border border-stone-800">
                  <BrickRating value={rating} interactive={true} onChange={setRating} />
                </div>
                <div className="mb-6">
                  <label className="block text-stone-300 font-bold mb-2 text-sm uppercase tracking-wider">{t('review.comment_label')} {t('review.comment_optional')}</label>
                  <textarea
                    value={comment}
                    ...
                    placeholder={t('profile.feedback_comment_placeholder')}
                  />
                </div>
                <div className="flex gap-3">
                  <button type="button" onClick={() => setFeedbackModalOpen(false)} className="flex-1 bg-stone-800 hover:bg-stone-700 text-white border border-stone-700 py-3 rounded-xl font-bold transition-colors">
                    {t('profile.cancel_button')}
                  </button>
                  <button type="submit" disabled={submittingFeedback} className="flex-1 bg-gold-600 hover:bg-gold-500 text-white py-3 rounded-xl font-bold transition-colors shadow-lg shadow-gold-600/20 disabled:opacity-50 disabled:cursor-not-allowed">
                    {submittingFeedback ? t('review.submitting') : t('review.submit_button')}
                  </button>
                </div>
              </form>
            </div>
