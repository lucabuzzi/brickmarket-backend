...
export default function AsteCardsHub() {
  ...
  return (
    <div className="page aste-cards-hub max-w-[1400px] mx-auto px-4 py-12 animate-fadeIn">
      <Link to="/aste" className="inline-flex items-center gap-2 text-stone-400 hover:text-white text-xs font-bold uppercase tracking-wider mb-8 transition-colors">
        <ArrowLeft size={14} /> {t('hubs.aste_cards.back')}
      </Link>
      ...
      <section className="relative mb-12 py-14 px-8 bg-[#050402] rounded-[48px] border border-white/5 shadow-2xl overflow-hidden text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto">
          <h1 className="text-3xl md:text-5xl font-black text-white mb-4 uppercase tracking-tighter leading-[0.9] italic">
            {t('hubs.aste_cards.title_pre')} <span className="text-gold-500">{t('hubs.aste_cards.title_accent')}</span>
          </h1>
          <p className="text-stone-500 text-base md:text-lg font-medium">
            {t('hubs.aste_cards.subtitle')}
          </p>
        </div>
      </section>
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {ANNUNCI_CARD_GAMES.map((game, idx) => (
          ...
          >
            <Link
              to={`/aste/carte-collezionabili/${game.slug}`}
