    ...
    <div className="p-6 max-w-[1200px] mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-stone-400">Analisi comportamentale e connessioni tra utenti della piattaforma.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Interaction Matrix / Heatmap Card */}
        <div className="lg:col-span-2 bg-[#120f0a] rounded-2xl border border-stone-800 p-6 shadow-2xl">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <Activity className="text-gold-400" size={20} /> Network Matrix
            </h2>
            <span className="text-[10px] font-black uppercase tracking-widest text-stone-500">Live Traffic Mapping</span>
          </div>

          <div className="aspect-video bg-stone-900/50 rounded-xl border border-stone-800/50 flex items-center justify-center relative overflow-hidden">
            ...
            <div className="relative z-10 grid grid-cols-5 md:grid-cols-8 gap-4 p-4 w-full h-full content-center">
              {interactions.slice(0, 24).map((inter, i) => (
                <div 
                  key={i}
                  className="aspect-square rounded-lg border border-gold-500/30 flex flex-col items-center justify-center transition-all hover:scale-110 hover:shadow-lg hover:shadow-gold-500/20"
                  style={{ 
                    backgroundColor: `rgba(212,175,55, ${Math.min(inter.trade_count * 0.15, 0.8)})`,
                    boxShadow: inter.trade_count > 3 ? '0 0 15px rgba(212,175,55, 0.4)' : 'none'
                  }}
            ...
            <div className="absolute bottom-4 left-4 right-4 flex justify-between text-[10px] font-bold text-stone-500 uppercase tracking-tighter">
          ...
          <div className="mt-6 flex gap-4">
            <div className="flex-1 bg-stone-900/80 p-4 rounded-xl border border-stone-800">
               <span className="text-stone-500 text-[10px] font-black uppercase block mb-1">Density Index</span>
            ...
            <div className="flex-1 bg-stone-900/80 p-4 rounded-xl border border-stone-800">
               <span className="text-stone-500 text-[10px] font-black uppercase block mb-1">Retention Rate</span>
               <div className="flex items-baseline gap-2">
                 <span className="text-xl font-black text-white">62%</span>
                 <span className="text-gold-400 text-xs font-bold">Stable</span>
               </div>
            </div>
          </div>
        </div>
        ...
        {/* Top Pairings List */}
        <div className="bg-[#120f0a] rounded-2xl border border-stone-800 p-6 shadow-2xl flex flex-col">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
          ...
          <div className="space-y-4 flex-1 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar">
            {topPairings.map((pair, i) => (
              <div key={i} className="p-3 rounded-xl bg-stone-900/50 border border-stone-800 hover:border-stone-700 transition-all group">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black uppercase text-gold-400 tracking-wider">High Frequency</span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                   <div className="flex flex-col">
                     <span className="text-[10px] text-stone-500 uppercase font-bold">Buyer</span>
                     <span className="text-sm font-bold text-white">{pair.buyer_username}</span>
                   </div>
                   <div className="h-px w-8 bg-stone-800" />
                   <div className="flex flex-col text-right">
                     <span className="text-[10px] text-stone-500 uppercase font-bold">Seller</span>
                     <span className="text-sm font-bold text-white">{pair.seller_username}</span>
                   </div>
                </div>

                <div className="mt-3 pt-2 border-t border-stone-800/50 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <span className="text-[10px] text-stone-500 font-bold uppercase">Volume Totale</span>
          ...
          <button className="mt-6 w-full py-3 rounded-xl bg-stone-800 hover:bg-stone-700 text-white text-xs font-bold uppercase tracking-widest transition-all border border-stone-700">
            Esporta Report PDF
          </button>
        </div>
      </div>
