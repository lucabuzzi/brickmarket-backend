...
export default function CatalogIndex() {
  ...
  const handleSearchSubmit = (e) => {
    ...
    setIsDropdownOpen(false);
    navigate(`/catalog/lego/search?q=${encodeURIComponent(searchQuery.trim())}`);
  };
            ...
            <AnimatePresence>
              {isDropdownOpen && searchQuery.length >= 2 && (
                ...
                >
                  ...
                  <div className="max-h-[480px] overflow-y-auto">
                    {searchResults.length > 0 ? (
                      ...
                      <div className="p-12 text-center">
                        <div className="text-stone-600 font-bold mb-2 uppercase tracking-widest">{t('catalog.archive_scan_complete')}</div>
                  ...
                  {searchResults.length > 0 && (
                    <Link
                      to={`/catalog/lego/search?q=${encodeURIComponent(searchQuery)}`}
                      onClick={() => setIsDropdownOpen(false)}
      ...
      <section>
        <div className="flex items-center justify-between mb-12">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-white uppercase tracking-tight">{t('catalog.recent_indexing_title')}</h2>
              <p className="text-stone-500 text-xs font-bold uppercase tracking-widest">{t('catalog.master_db_sync')}</p>
            </div>
          </div>
        </div>
